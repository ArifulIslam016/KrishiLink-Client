...
const Interest = ({ interest }) => {
  ...
  const handleAccept = (cropId, interestId, quantity) => {
    ...
    Instance.put("/iterest-update", { cropId, interestId, quantity })
      .then((data) => {
        // console.log(data.data)
        setAcceptedInteres((prev) =>
          prev.map((singleInterest) =>
            singleInterest._id === interestId
              ? { ...singleInterest, Status: "accepted" }
              : singleInterest
          )
        );
      })
    ...
    Instance.put("/reject-Update", {
      ...
      .then((data) => {
        ...
        setAcceptedInteres((prev) =>
          prev.map((singleInterest) =>
            singleInterest._id === interestId
              ? { ...singleInterest, Status: "rejected" }
              : singleInterest
          )
        );
      })
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      <h1 className="text-2xl font-bold">
        Total {accpetedInterst.length} interst found
      </h1>
      {interest.length === 0 ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold">
                            {singleInterest.userName}{" "}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td>
                      {singleInterest.quantity} <br />
                    </td>
                    <td>
                      {singleInterest?.totalPrice
                        ? singleInterest.totalPrice
                        : ""}
                      <br />
                    </td>
                    <td>
                      {singleInterest.Status == "pending" ? (
                      ...
                      {singleInterest.Status == "pending" ? (
                        <div className="space-x-2 flex">
                          <button
                            onClick={() =>
                              handleAccept(
                                singleInterest.CropId,
                                singleInterest._id,
                                singleInterest.quantity
                              )
                            }
                            className="btn btn-outline text-green-500 btn-xs"
                          >
                            Accept
                          </button>
                          <button
                        ...
                        <h1>Actions are availabe only for pending interests</h1>
                      )}
                    </th>
                  </tr>
                );
